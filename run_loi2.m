...
%% Test Button Box %%
if defaults.testbb, bbtester(inputDevice, w.win); end
...
motionTex = Screen('MakeTexture', w.win, imread([defaults.path.stim filesep 'motion_reminder.jpg']));
...
try
    ...
    %% Present Reminder %%
    Screen('DrawTexture',w.win, motionTex); Screen('Flip',w.win); WaitSecs(3); 
    ...
    for b = 1:nBlocks 
        ...
        for t = 1:nTrialsBlock 
            ...
            doquit = ptb_get_force_quit(inputDevice, KbName(defaults.escape), winopp*.99);
            if doquit
...
try
    ...
    bob_sendemail({'bobspunt@gmail.com'},'peers asd loi behavioral data','see attached', [defaults.path.data filesep outfile]);
    disp('All done!');
catch
